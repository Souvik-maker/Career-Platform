...
# Fetch DB URI from .env or Streamlit Cloud Secrets
DATABASE_URL = os.getenv("DATABASE_URL")

def get_connection():
    """Establishes a connection to the remote Supabase PostgreSQL database."""
    if not DATABASE_URL:
        raise ValueError("[DB ERROR] DATABASE_URL is missing. Please set it in .env or Streamlit Secrets.")
    conn = psycopg2.connect(DATABASE_URL)
    return conn

def init_db():
    """Initializes PostgreSQL tables in Supabase."""
    try:
        conn = get_connection()
        cursor = conn.cursor()
        ...
        cursor.close()
        conn.close()
        print("[DB SUCCESS] Supabase PostgreSQL database initialized successfully!")
...
def save_initial_resume(file_name: str, resume_text: str) -> int:
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO resumes (file_name, resume_text) VALUES (%s, %s) RETURNING id",
...
def update_resume_analysis(resume_id: int, skills: list, certifications: list, experience: str, roles: list):
    ...
    try:
        ...
        cursor.execute("""
            UPDATE resumes 
            SET parsed_skills = %s, parsed_certifications = %s, experience = %s, recommended_roles = %s
            WHERE id = %s
